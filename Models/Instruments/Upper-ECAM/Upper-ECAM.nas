...
var eng1_n3 = props.globals.getNode("/engines/engine[0]/n3", 1);
var eng2_n3 = props.globals.getNode("/engines/engine[1]/n3", 1);
...
var canvas_upperECAM_base = {
	...
	update: func() {
		...
		if (acess.getValue() >= 110 and du3_lgt.getValue() > 0.01) {
			if (du3_test_time.getValue() + du3_test_amount.getValue() >= elapsedtime) {
				...
				if (cur_eng_option == "GE") {
					upperECAM_ge.page.show();
					upperECAM_pwrr.page.hide();
					upperECAM_ge.update();
				} else if (cur_eng_option == "PW" or cur_eng_option == "RR") {
					upperECAM_ge.page.hide();
	...
	updateBase: func() {
		...
		cur_eng_option = eng_option.getValue();
		if (rev_1_cur >= 0.01 and eng1_n1mode.getValue() == 1 and cur_eng_option == "GE") {
			me["REV1"].show();
			me["REV1-box"].show();
		} else if (rev_1_cur >= 0.01 and eng1_eprmode.getValue() == 1 == 1 and (cur_eng_option == "PW" or cur_eng_option == "RR")) {
			me["REV1"].show();
			me["REV1-box"].show();
		} else {
			me["REV1"].hide();
		...
		if (rev_2_cur >= 0.01 and eng2_n1mode.getValue() == 1 and cur_eng_option == "GE") {
			me["REV2"].show();
			me["REV2-box"].show();
		} else if (rev_2_cur >= 0.01 and eng2_eprmode.getValue() == 1 and (cur_eng_option == "PW" or cur_eng_option == "RR")) {
			me["REV2"].show();
			me["REV2-box"].show();
		} else {
			me["REV2"].hide();
...
var canvas_upperECAM_pwrr = {
	...
	update: func() {
		...
		# N2/N3
		if (eng_option.getValue() == "RR") {
			me["N21Label"].setText("N3");
			me["N22Label"].setText("N3");
			me["N21"].setText(sprintf("%s", math.floor(eng1_n3.getValue() + 0.05)));
			me["N21-decimal"].setText(sprintf("%s", int(10 * math.mod(eng1_n3.getValue() + 0.05, 1))));
			me["N22"].setText(sprintf("%s", math.floor(eng2_n3.getValue() + 0.05)));
			me["N22-decimal"].setText(sprintf("%s", int(10 * math.mod(eng2_n3.getValue() + 0.05, 1))));
